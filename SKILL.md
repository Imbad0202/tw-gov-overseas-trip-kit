...
10. [個案報支方向指引（延返 / 艙等 / 保險等）](#個案報支方向指引延返--艙等--保險等)
11. [docx 轉 PDF 指引](#docx-轉-pdf-指引)
12. [法規版本鎖定提醒與免責聲明](#法規版本鎖定提醒與免責聲明)
...
填 `itinerary` 時，段內資訊（幾點、搭什麼、多少錢）通常都會寫到，容易漏的是**時段與時段之間的接縫**：在哪一站下車、出站後怎麼走到下一段的乘車處、當日行程結束走回哪個車站。手冊是給人在現場照著走的，接縫缺漏時使用者只能當場重查。

含交通段的 item 建議用 `note` 補三件事：

1. **下車點**——哪一站、哪個出口下車
2. **接續動線**——出站後往哪走、走幾分鐘、下一段在哪個乘車處上車（月台／站牌編號查得到官方來源就寫，查不到就寫「站前站牌＋現場詢問處」，不要猜編號）
3. **收尾回程**——當日最後一個活動結束後，走回哪個車站、怎麼回住宿點（最後一段最常被漏）

步行段的分鐘數用「道路距離」驗證，不要用地圖直線距離：郊區的道路距離常是直線的 1.5–2 倍，以實走記錄或當地官方說明為準。官方交通指南寫「建議搭車」的區段，先查清徒步的真實成本再決定改用步行。

完成後自檢：對行程中每個移動問三題——哪站下車？下一段在哪上車、怎麼走過去？結束後回哪裡？答不出來的就是缺漏。
...
出差常見個案——延後返國 / 不可抗力滯留、搭機艙等、機票單據、供膳宿扣補、保險、雜費禮品、匯率、進修研習等——的**方向性**參考，整理於獨立指引：

**→ [docs/延返與個案指引.md](docs/延返與個案指引.md)**

該指引按 A–H 八區組織，每題以「**方向 → 需向官方確認的點 → 工具端如何反映**」呈現。定位是**方向性參考，非個案認定**：只講「往哪個方向想、該確認什麼」，不下結論、不給可照抄金額比例，凡個案認定一律導回主計總處與貴機關核定。

要查主計總處歷年函釋與主計長信箱答復的個案，用 **[docs/解釋彙編逐案索引.md](docs/解釋彙編逐案索引.md)**：依要點各點編號列出 107.4 修編《解釋彙編》全部案例，附函釋文號、要旨與「與現行 114.05.13 條文的差異」。引用時附文號並標差異；彙編本身不是法規，不覆蓋現行條文。

以延後返國為例：颱風等**非可歸責事由**滯留，報支要點第 12 點訂有相關規定（經確實證明、機關首長核准者得按日報支）；旺季訂不到票等**可歸責事由**多不得報支。工具端於延返獲核准後，以該 `segment` 的 `approved_extension: true` 反映（見〈計算器 scope 邊界〉一節與 `calc.per_diem` 之 `approved_days` / `approved_extension`）。詳見上開指引。
