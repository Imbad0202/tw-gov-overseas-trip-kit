...
description: >
  ...
  （可選，逐日議程／住宿／緊急聯絡／注意事項，可印 PDF）。使用者帶入自己機關的資料，
  日支數額表由使用者填當年度官方版本（不內建）。觸發：產出國報告、出國報告書、審核表、
  經費規劃表、行前手冊、日支費計算、國外出差旅費、出差核銷文件。
license: MIT
---
...
4. [充實報告本文（目的／過程／心得及建議）](#充實報告本文目的過程心得及建議)
5. [啟動流程：帶入你機關的範例](#啟動流程帶入你機關的範例)
6. [日支基準額填值流程（`per_diem_base`）](#日支基準額填值流程per_diem_base)
7. [B 類人工值填入](#b-類人工值填入)
8. [計算器 scope 邊界](#計算器-scope-邊界)
9. [docx 轉 PDF 指引](#docx-轉-pdf-指引)
10. [法規版本鎖定提醒與免責聲明](#法規版本鎖定提醒與免責聲明)
